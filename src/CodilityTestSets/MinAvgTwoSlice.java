...
public class MinAvgTwoSlice {
    public int solution(int[] A) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            if (i < n - 2) {
             ...
             // Check slice of length 4
            if (i< n - 3) {
                double avg4 = (A[i] + A[i + 1] + A[i + 2] + A[i + 3]) / 4.0;
                if (avg4 < minAvg) {
                    minAvg = avg4;
                    minIndex = i;
                }
            ...
            // Check slice of length 5
            if (i< n - 4) {
                double avg5 = (A[i] + A[i + 1] + A[i + 2] + A[i + 3] + A[i + 4]) / 5.0;
                if (avg5 < minAvg) {
                    minAvg = avg5;
                    minIndex = i;
                }
    ...
    public static void main(String[] args) {
        ...
        int[] A = {4, 2, 2, 5, 1, 5, 8};
        int result = mats.solution(A);
        System.out.println("Starting position of the slice with the minimal average is: " + result); // Output: 1
    }
